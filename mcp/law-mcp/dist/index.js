...
// --- Tool 1: 법령 검색 ---
server.tool('search_law', '법령명으로 현행 **법령**(법률·대통령령·총리령·부령)을 검색하여 법령ID 목록을 반환합니다. ' +
    '법령ID는 get_law_text 또는 get_law_article 호출에 사용합니다.\n' +
    '⚠ 본 도구는 **행정규칙(고시·훈령·예규·세칙)은 검색하지 않습니다**. ' +
    '"전자금융감독규정", "○○ 시행세칙", "○○ 고시" 같은 행정규칙명은 별도 도구 **search_admin_rule**을 사용하세요.', { query: z.string().describe('검색할 법령명 (예: "전자금융거래법", "전자금융거래법 시행령"). 행정규칙명은 search_admin_rule 사용.') }, async ({ query }) => {
    try {
        ...
        const text = results.map(r => `[법령ID: ${r.법령ID}] ${r.법령명한글} (${r.법령구분명}) | 시행일: ${r.시행일자} | 소관: ${r.소관부처명}`).join('\n');
...
// --- Tool 2: 법령체계도 조회 ---
server.tool('get_law_hierarchy', '법령ID로 법령체계도를 조회합니다. ' +
    ...
    '행정규칙 본문은 행정규칙일련번호로 get_admin_rule_text를 호출하세요.', { law_id: z.string().describe('법령ID (search_law 결과에서 획득, 예: "010199")') }, async ({ law_id }) => {
    try {
        const h = await getLawHierarchy(law_id);
        const lines = [`# ${h.법령명} 법령체계도 (법령ID: ${h.법령ID})`, ''];
        const formatTier = (label, tier) => {
            if (!tier)
                return;
...
// --- Tool 2.5: 행정규칙 검색 ---
server.tool('search_admin_rule', '행정규칙명으로 현행 행정규칙(고시·훈령·예규·세칙 등)을 검색하여 일련번호 목록을 반환합니다. ' +
    '일련번호는 get_admin_rule_text 또는 get_admin_rule_article 호출에 사용합니다. ' +
    '예: "전자금융감독규정", "전자금융감독규정시행세칙", "금융기관 검사 및 제재에 관한 규정". ' +
    '부분일치로 동작하므로 짧은 키워드(예: "전자금융감독")로 시작하는 것이 안전합니다.', { query: z.string().describe('검색할 행정규칙명 (예: "전자금융감독규정", "내부통제기준")') }, async ({ query }) => {
    try {
        const results = await searchAdminRule(query);
        if (results.length === 0) {
            return { content: [{ type: 'text', text: `"${query}"에 해당하는 현행 행정규칙을 찾을 수 없습니다.` }] };
        }
        const text = results.map(r => `[일련번호: ${r.행정규칙일련번호}] ${r.행정규칙명} (${r.행정규칙종류}) | 시행: ${r.시행일자} | 발령: ${r.발령일자} | 소관: ${r.소관부처명}`).join('\n');
        return { content: [{ type: 'text', text }] };
    }
    catch (e) {
        return { content: [{ type: 'text', text: `오류: ${e.message}` }], isError: true };
...
// --- Tool 3: 행정규칙 본문 조회 ---
server.tool('get_admin_rule_text', '행정규칙 일련번호로 행정규칙(고시·세칙·훈령 등) 본문을 조회합니다. ' +
    '일련번호는 search_admin_rule 또는 get_law_hierarchy 결과에서 획득하세요.', { rule_serial_no: z.string().describe('행정규칙 일련번호 (예: "2100000274812")') }, async ({ rule_serial_no }) => {
    try {
...
server.tool('get_admin_rule_article', '행정규칙 일련번호와 조번호로 특정 조문만 조회합니다. ' +
    ...
    try {
        ...
        if (!svc)
            throw new Error(`행정규칙 일련번호 ${rule_serial_no}를 찾을 수 없습니다.`);
        const lines = Array.isArray(svc.조문내용) ? svc.조문내용 : [svc.조문내용];
        ...
        for (let i = startIdx; i < lines.length; i++) {
            if (i > startIdx && nextArticlePattern.test(String(lines[i])))
                break;
...
// --- Tool 5: 법령 전문 조회 ---
server.tool('get_law_text', '법령ID로 현행 법령의 전체 조문을 조회합니다. ' +
    '분량이 크므로 특정 조문만 필요한 경우 get_law_article을 사용하세요.', { law_id: z.string().describe('법령ID (search_law 결과에서 획득, 예: "010199")') }, async ({ law_id }) => {
    try {
        const result = await getLawText(law_id);
        const lines = [
            `# ${result.법령명}`,
...
// --- Tool 5: 특정 조문 조회 ---
server.tool('get_law_article', '법령ID와 조번호로 특정 조문을 상세 조회합니다. ' +
    '조번호는 article_number, 가지번호("6조의2"의 "2")는 sub_number로 입력합니다. ' +
    'article_number에 "6의2" 같은 문자열을 넣어도 가지번호가 자동 인식됩니다.', {
...
server.tool('get_admin_rule_toc', '행정규칙 일련번호로 행정규칙의 조문 번호·제목 목차만 조회합니다. ' +
    '조문 내용은 포함하지 않아 가볍습니다. ' +
    '목차를 보고 필요한 내용을 파악한 뒤 get_admin_rule_text로 전문을 조회하세요. ' +
    '일련번호는 get_law_hierarchy 결과에서 획득하세요.', { rule_serial_no: z.string().describe('행정규칙 일련번호 (예: "2100000274812")') }, async ({ rule_serial_no }) => {
    try {
        const res = await (await import('axios')).default.get('http://www.law.go.kr/DRF/lawService.do', {
            params: { OC: process.env.LAW_OC, target: 'admrul', ID: rule_serial_no, type: 'JSON' },
        });
        ...
        const info = svc.행정규칙기본정보 ?? {};
        const lines = (Array.isArray(svc.조문내용) ? svc.조문내용 : [svc.조문내용])
            .filter((line) => /^제\d+조/.test(String(line)));
        const header = [
            `# ${info.행정규칙명 ?? ''} 조문 목차`,
...
// --- Tool 7: 법령 조문 목차 조회 ---
server.tool('get_law_toc', '법령ID로 법령의 조문 번호와 제목 목록(목차)만 조회합니다. ' +
    '조문 내용은 포함하지 않아 가볍습니다. ' +
    '목차를 보고 필요한 조문 번호를 파악한 뒤 get_law_article로 해당 조문만 조회하세요.', { law_id: z.string().describe('법령ID (search_law 결과에서 획득, 예: "010199")') }, async ({ law_id }) => {
    try {
        const result = await getLawText(law_id);
        const lines = [
            `# ${result.법령명} 조문 목차`,
...
function parseArticleRef(primary, primaryAlias, sub, subAlias) {
    ...
    if (subNum === 0) {
        ...
        if (typeof rawSub === 'number')
            ...
            subNum = parseInt(rawSub.match(/\d+/)?.[0] ?? '0', 10);
    }
...
function formatArticle(article) {
    const lines = [`제${article.조번호}조${article.조제목 ? `(${article.조제목})` : ''}`];
    if (article.조문내용)
        lines.push(article.조문내용);
