...
### 수동 큐레이션 오버라이드 (= Kustomize base+overlay / git rebase)
위 판단 보정(주로 rdb 연결)을 **규정 갱신과 무관하게 영속**시키는 레이어. `pipeline/overrides.py`:
- `rdb.json` = 순수 자동(결정론 베이스), `overrides.json` = 사람 델타(`{rdb:{add,remove}}`). **적재 = 자동 + 오버라이드**.
- **capture**: GUI '오버라이드 저장' 버튼(또는 `python -m pipeline.overrides <code>`) → 라이브 DB.rdb ⊖ rdb.json → overrides.json 박제.
- **재적용**: `run --apply`(특히 `--force` 갱신) 적재 시 `apply_rdb_overrides`가 자동 재적용 → 큐레이션 생존. **안정 ID(A37/S17…)가 이걸 가능케 함**(순번ID였으면 불가).
- **충돌**: add 대상 노드가 갱신으로 사라지면 스킵+경고(= rebase 충돌). overrides.json 은 **git 추적**(큐레이션=소스).
- 적용 범위: 현재 **rdb 한정**(본문은 API 권위라 갱신 시 새 본문이 맞음). 편집은 반드시 GUI 편집기/capture 경유(Workbench 직접수정도 capture가 잡지만 기록 누락 위험).
