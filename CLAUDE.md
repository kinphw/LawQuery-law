...
- **연혁(구버전) 고정**: law 단은 `id` 대신 `mst`+`ef_yd`(시행일자) 주면 그 시행일 버전 조회. admrul 단은 `id`에 연혁 일련번호 직접. (구버전 일련번호/MST는 `lawSearch.do?nw=1,2`(연혁) 검색으로 획득.) DB가 특정 시행일 버전이면 그 버전으로 베이스를 떠야 오버라이드가 깨끗함.
- **시행예정(미시행 개정)**: law 단에 `"sched": true` → build가 시행예정 버전(`find_sched_version`, nw=2)을 받아 **현행과 조별 내용 diff** → 변경/신설 조에 `content_<tier>_sched`(미래본문)+`sched_date`(시행일) 채움(신설 조는 행도 추가). 프론트가 시행예정을 달리 표시. ⚠ 시행예정 버전은 모든 조의 `조문시행일자`가 버전 시행일로 같아서 **내용 diff로 식별**(조문시행일자로는 못 가림). 오버라이드/분리가 sched 컬럼 자동 보존.
...
## 실행 — 허브(`Dashboard.pyw`)가 단일 현관
- **허브(권장)**: 루트 `Dashboard.pyw` 더블클릭 → 로컬 웹 대시보드(인테이크·파이프라인·도구). → `dashboard/README.md`
  - 인테이크 폼은 허브 웹으로 흡수(구 `NewLaw.pyw` → `legacy/`). 파이프라인 run/verify/replicate 도 허브에서 SSE 로그.
- GUI 편집기: 허브 '도구' 탭에서 런치 (또는 `legacy/LawEditor.pyw` 더블클릭 — 인자 없으면 GUI).
- CLI: `python legacy/LawEditor.pyw {template|ingest|export|gui}` 또는 `python run.py …`. `run.py` 는 CLI 구현 모듈.
