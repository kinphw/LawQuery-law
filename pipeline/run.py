...
def _load_data_for(code: str, stages: list) -> dict:
    """선택 단계의 산출물 → 적재용 data dict(해당 시트만). rdb는 자동+오버라이드."""
    from pipeline.overrides import load_overrides, apply_rdb_overrides
    data = read_artifact(code, "data.json")
    out: dict = {}
    if "build" in stages:
        for s in STAGE_SHEETS["build"]:
            out[s] = data[s]
    if "rdb" in stages:
        valid = {row[f"id_{t}"] for t in ("a", "e", "s", "r")
                 for row in data[t] if row.get(f"id_{t}")}
        final = apply_rdb_overrides(read_artifact(code, "rdb.json")["edges"],
                                    load_overrides(code), valid)
        out["rdb"] = [{"id": None, **e} for e in final]
    if "annex" in stages:
        out["annex"] = read_artifact(code, "annex.json")
    if "ref" in stages:
        out["ref"] = read_artifact(code, "ref.json")
    if "penalty" in stages:
        pen = read_artifact(code, "penalty.json")
        out["penalty"], out["penalty_a"], out["penalty_e"] = (
