...
```jsonc
{
  ...
  "labels": {                  // 단 종류명(감독규정/시행세칙 등) — 단수 기본값을 단별로 수정 가능
    "a": "법", "e": "시행령",  //   기본값과 같으면 생략 가능. job.json 의 sources.<tier>.short 로 반영.
    "s": "감독규정", "r": "감독규정"  //   예: s·r 둘 다 감독규정인 법(세칙 없이 제재규정이 r 슬롯)
  },
...
2. `jobs/<code>/job.json` 작성 — 스키마는 `../CLAUDE.md` ★섹션. `tiers==5` 면 별표(B) 포함,
   `options.sched==true` 면 해당 law 단에 `"sched": true`. 각 `sources.<tier>.short` 는
   `labels.<tier>`(있으면) 그대로 — s·r 둘 다 "감독규정" 같은 사례를 라벨로 지정해 둠.
