...
# 단(tier) 정의: 코드 → (단 종류명 기본값, 종류 힌트). 슬롯 코드(a/e/s/r/b)는 위치 기반 —
# 단수에 따라 의미가 한 칸씩 시프트한다(jobs/g·j=4단, jobs/s·y=5단 실측 기준).
#   4단: a=법  e=시행령  s=감독규정  r=시행세칙
#   5단: a=법  e=시행령  s=시행규칙  r=감독규정  b=시행세칙  (시행령↔감독규정 사이 시행규칙 삽입)
# label 은 단수 기준 *기본값*일 뿐 — 폼에서 단별로 수정 가능(예: s·r 둘 다 "감독규정"인 법).
# 수정값은 intake JSON 의 labels 로 저장. 슬롯 코드(A/E/S/R/B)는 프론트가 code 로 붙임.
TIERS_BY_RANK = {
    4: [
        ("a", "법", "law"),
        ("e", "시행령", "law"),
